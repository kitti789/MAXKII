...
void SDM::begin(const byte nodeID)
{
    ...
    //Serial.print("Pin Controll is ");
    //Serial.println(_ctlPin);

   // _sdmSerial.begin(_baudRate, _protocol);
    _sdmSerial.begin(_baudRate);
    //Serial.print("Pin baudrate  is ");
...
SDM::Status SDM::getRawData(byte* reData, byte* request, size_t size)
{
	...
	//delay(5); //< [WARNING] This delay is VERY IMPORTANT
    _sdmSerial.write(request, size);
    delay(10); //< [WARNING] This delay is VERY IMPORTANT
    digitalWrite(_ctlPin, RS485_RECEIVE);

    delay(100); //< [WARNING] This delay is VERY IMPORTANT
    byte i = 0;
...
float SDM::getValue(SDM::Type type)
{
	...
	//sprintf(tempdataprint, "type process is %d %x %x", (SDM::Type)(int)type,_highAddrByte,_lowAddrByte);
    ...
    memset(reData, 0, (size_t)9);
    if (getRawData(reData, rCmd.cmd, rCmd.size) == ERR_READ_RS485) {
        //Serial.println(8);
		//sprintf(tempdataprint,"Error Mb \r\n");
        return (float) 0;
    }

    //sprintf(tempdataprint, "%x %x %x %x %x %x %x %x %x", reData[0], reData[1],reData[2], reData[3], reData[4], reData[5],reData[6], reData[7], reData[8]);
